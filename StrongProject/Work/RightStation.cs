...
namespace StrongProject
{
	...
	{   /// <summary>
		...
		public short Step1(object o)
		{
			...
			while (true)
			{
				if (NewCtrlCardV0.GetInputIoBitStatus(tag_stationName, "右启动", out start_R) != 0)
				{
					UserControl_LogOut.OutLog("获取右启动按钮输入状态失败!", 0);
					return -1;
				}
		...
		public short Step8(object o)
		{
			...
			{
				if (NewCtrlCardV0.AxisAbsoluteMoveAndWaitStopVrf("空跑点位", "右Y轴", "右切电池1位", 1) != 0)
				{
					return -1;
				}
			}
			else
			{
				...
				AxisConfig axisC = StationManage.FindAxis(stationM, "右Y轴");
		...
		public short Step10(object o)
		{
			...
			if (Global.WorkVar.bEmptyRun)
			{
				if (NewCtrlCardV0.AxisAbsoluteMoveAndWaitStopVrf("空跑点位", "右Y轴", "右切电池2位", 1) != 0)
				{
					return -1;
				}
			}
		...
		public short Init()
		{
			...
			PointAggregate _Step1 = pointMotion.FindPoint(tag_stationName, "等待右工位双启", 1);
			if (_Step1 == null)
			{
				return -1;
			}
			...
			PointAggregate _Step3 = pointMotion.FindPoint(tag_stationName, "右Y到测试位", 3);
			if (_Step3 == null)
			{
				return -1;
			}
			_Step3.tag_BeginFun = Step3;
			PointAggregate _Step4 = pointMotion.FindPoint(tag_stationName, "右测试离合气缸下降", 4);
			if (_Step4 == null)
			{
				return -1;
			}
			...
			PointAggregate _Step6 = pointMotion.FindPoint(tag_stationName, "右测试离合气缸上升", 6);
			if (_Step6 == null)
			{
				return -1;
			}
			_Step6.tag_EndFun = Step6;
			PointAggregate _Step7 = pointMotion.FindPoint(tag_stationName, "右Y到拍照位并等待拍照", 7);
			if (_Step7 == null)
			{
				return -1;
			}
			_Step7.tag_EndFun = Step7;
			PointAggregate _Step8 = pointMotion.FindPoint(tag_stationName, "右Y到切电池1位", 8);
			if (_Step8 == null)
			{
				return -1;
			}
			...
			PointAggregate _Step10 = pointMotion.FindPoint(tag_stationName, "右Y到切电池2位", 10);
			if (_Step10 == null)
			{
				return -1;
			}
			...
			PointAggregate _Step12 = pointMotion.FindPoint(tag_stationName, "右Y到待料位", 12);
			if (_Step12 == null)
			{
				return -1;
			}
